...
let rec terrainAddStairs ~dir rooms m =
    ...
    match rooms with
        ...
        | rooms ->
            ...
            if Matrix.get m p |> Fun.compose Option.is_some Map.isStairs then
...
let rec placeRoomCreatures rooms state =
    match rooms with
        | [] -> state
        | r::ro ->
            let s' = if R.oneIn 3 then Ai.spawnCreatures ~preferNear:RandomFirstOnly ~room:(Some r) state else state in
...
let genLevelFinal state =
    SL.levelAdd (Levels.levelFinal ()) state
